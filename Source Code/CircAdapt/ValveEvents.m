...
q_aux       = Get('Valve','q',VAvalve(ind));
q           = q_aux(reref,:);
qDot_aux    = Get('Valve','qDot',VAvalve(ind));
qDot        = qDot_aux(reref,:);
...
q_aux       = Get('Valve','q',AVvalve(ind));
q           = q_aux(reref,:);
qDot_aux    = Get('Valve','qDot',AVvalve(ind));
qDot        = qDot_aux(reref,:);

% Atrial Activation
C_aux       = Get('Patch','C',Atr(ind));
C           = C_aux(reref,:);
CDot_aux    = Get('Patch','CDot',Atr(ind));
CDot        = CDot_aux(reref,:);
...
% vAvMaxE     = max(q(round(length(q)/2):end))/Get('Valve','AOpen',ValveAv); % E-peak velocity
