...
% CorArtVen->Cavity
CorArtVenName   = Net.CorArtVen.Name; % getting ArtVen names
CorArtVen2Cavity= strcat(CorArtVenName,{'Ar'}); % arterial coupled cavity names
Aux=[strcat(CorArtVenName,{'Ar'});strcat(CorArtVenName,{'Ve'})];
CavityName= [CavityName,Aux(:)']; % names of arterial, myocardial and venous coupled cavities
...
P.CorArtVen = Naming('CorArtVen',CorArtVenName );
...
P.CorArtVen.iCavity = Get('Cavity','Index',CorArtVen2Cavity );
...
P.CorArtVen.iNode= P.Cavity.iNode(P.CorArtVen.iCavity);
...
iAvC  = Get('CorArtVen','Index',P.Net.Bag.CorArtVen{iBag} );
iCCh  = P.Chamber.iCavity(iCh)    ;
iCTr1 = P.TriSeg.iCavity(iTr)     ; iCTr=[iCTr1,iCTr1+1];
iCAv1 = P.ArtVen.iCavity(iAv)     ; iCAv=[iCAv1,iCAv1+1];
iCAvC1= P.CorArtVen.iCavity(iAvC) ; iCAvC=[iCAvC1,iCAvC1+1];
P.Bag.iCavity{iBag}= [iCCh,iCTr,iCAv,iCAvC];
